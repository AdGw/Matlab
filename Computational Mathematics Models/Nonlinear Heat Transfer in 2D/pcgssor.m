...
for j = 2:n
    for i = 2:n
        r(i,j) = rhs(i,j)-(ac(i,j)*up(i,j)+aw(i,j)*up(i-1,j)+ae(i,j)*...
        up(i+1,j)++as(i,j)*up(i,j-1)+an(i,j)*up(i,j+1));
    end
end
error = 1. ;
m = 0;
...
while ((error>.0001)&&(m<200))
    ...
    for j= 2:n
        for i = 2:n
            rhat(i,j) = w*(r(i,j)-aw(i,j)*...
            rhat(i-1,j)-as(i,j)*rhat(i,j-1))/ac(i,j);
        end
    end
    for j= 2:n
        for i= 2:n
    ...
    for j= n:-1:2
        for i= n:-1:2
            rhat(i,j) = w*(rhat(i,j)-ae(i,j)*...
            rhat(i+1,j)-an(i,j)*rhat(i,j+1))/ac(i,j);
        end
    end
